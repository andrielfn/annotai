...
`;var p4=["blue","indigo","cyan","green","yellow","orange","red"],c4="Also show annotations the agent resolved or dismissed. Hidden by default so the page only shows open feedback.",m4="Show markers from every page, not just the current one. Off by default so a marker only appears where its element lives.",w3=n,b3=({label:Q,tip:X,ariaLabel:$,on:q,onToggle:J,onTip:Z,onUntip:z})=>V`<div class="set-row">
  ...
  </div>`,y3=({theme:Q,accent:X,version:$,showResolved:q,showAllPages:J,connected:Z,lastSeenMsAgo:z,copied:W,regCommand:F,onToggleTheme:K,onSetAccent:G,onToggleShowResolved:N,onToggleShowAllPages:Y,onTip:B,onUntip:P,onCopy:H})=>V`<div class="settings">
    <div class="set-head">
...
`;var T3=[{key:"pending",label:"Pending",icon:M3},{key:"acknowledged",label:"Acknowledged",icon:I3},{key:"resolved",label:"Resolved",icon:U3},{key:"dismissed",label:"Dismissed",icon:_3}],$Q=Object.fromEntries(T3.map((Q)=>[Q.key,Q])),r4=(Q)=>$Q[Q]??$Q.pending,l4=(Q)=>{let X=Date.parse(Q??"");return Number.isNaN(X)?null:n(Date.now()-X)},i4=(Q,X)=>T3.map(({key:$})=>({key:$,n:Q.filter((q)=>q.status===$).length})).filter(($)=>$.n>0||X.includes($.key)),o4=(Q,X)=>{let $=r4(Q.status),q=s(Q.url),J=l4(Q.inserted_at),Z=Array.isArray(Q.thread)?Q.thread.filter((z)=>z.role==="human").length:0;return V`<button class="hist-card" data-status=${$.key} @click=${()=>X(Q)}>
    <span class="hist-top">
  ...
  </button>`},S3=({annotations:Q,filter:X,onJump:$,onToggleFilter:q,onTip:J,onUntip:Z})=>{let z=X??[],W=[...Q].reverse(),F=z.length?W.filter((G)=>z.includes(G.status)):W,K=i4(Q,z);return V`<div class="history">
    <div class="hist-head">
    ...
    <div class="hist-list">
      ${F.length?F.map((G)=>o4(G,$)):V`<div class="hist-none">${Q.length?"No matching annotations.":"No annotations yet."}</div>`}
    </div>
  </div>`},v3=D`
  .history {
    position: fixed;
